...
const MenCollection = () => {
  const banner = useBanner('men-collection')
  ...
  useEffect(() => {
    const fetchCollections = async () => {
      try {
        ...
        if (response.data.success) setCollections(response.data.collections.filter((item) => item.gender === 'Men'))
      } catch (error) {
        console.log(error)
      } finally {
        setLoaded(true)
  ...
  return (
    <main className='collection-view-all'>
      <section className='collection-view-all-hero' style={banner?.image ? { backgroundImage: `linear-gradient(rgba(0, 0, 0, .58), rgba(0, 0, 0, .58)), url(${banner.image})` } : undefined}>
        ...
        <h1>{banner?.title || 'MEN COLLECTION'}</h1>
        {(!banner || banner.subtitle) && <span>{banner?.subtitle || 'Explore every Distressed collection created for men.'}</span>}
      </section>
        ...
        <section className='collection-view-all-section'>
          ...
          {loaded && <div className='collection-view-all-grid'>
            {collections.map((card) => {
              ...
              return (
                <button key={card._id} type='button' onClick={() => window.location.href = `/men-collection/${collectionRoute}`} className='collection-view-all-card'>
